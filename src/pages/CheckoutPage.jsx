    ...
    return (
      <section className="px-5 py-16 md:px-10 md:py-20">
        <div className="mx-auto max-w-3xl rounded-3xl border border-sand-200 bg-sand-100 p-8 text-center">
          ...
          <p className="mb-3 font-sans text-xs font-semibold uppercase tracking-[0.2em] text-clay-700">Order Confirmed</p>
          <h1 className="mb-4 text-5xl font-light text-forest-900">Thanks for your order.</h1>
          <p className="mb-6 font-sans text-forest-600">
            {submittedOrder.firstName || 'Customer'}, your order <strong>{submittedOrder.id}</strong> was placed on {submittedOrder.placedAt}.
          </p>
          <div className="mx-auto mb-7 max-w-sm rounded-2xl border border-sand-300 bg-sand-50 p-4 text-left font-sans text-sm text-forest-700">
            ...
            {submittedOrder.discount > 0 ? <p>Discount: -{formatUSD(submittedOrder.discount)}</p> : null}
  ...
  if (lineItems.length === 0) {
    return (
      <section className="px-5 py-20 md:px-10">
        <div className="mx-auto max-w-3xl rounded-3xl border border-sand-200 bg-sand-100 p-8 text-center">
          <p className="mb-3 font-sans text-xs font-semibold uppercase tracking-[0.2em] text-clay-700">Checkout</p>
    ...
    <section className="px-5 py-12 md:px-10 md:py-16">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-7 lg:grid-cols-[1.1fr_0.9fr]">
          <form onSubmit={handleSubmit} className="space-y-5 rounded-3xl border border-sand-200 bg-sand-100 p-5 md:p-6" noValidate>
            <h2 className="text-4xl font-light text-forest-900">Contact & Shipping</h2>

            <label className="block">
              <span className="mb-1.5 block font-sans text-xs font-semibold uppercase tracking-[0.18em] text-forest-600">Email</span>
              <input
                type="email"
                name="email"
                value={form.email}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="block">
                <span className="mb-1.5 block font-sans text-xs font-semibold uppercase tracking-[0.18em] text-forest-600">First Name</span>
                <input
                  type="text"
                  name="firstName"
              ...
              <label className="block">
                <span className="mb-1.5 block font-sans text-xs font-semibold uppercase tracking-[0.18em] text-forest-600">Last Name</span>
                <input
                  type="text"
                  name="lastName"
                  value={form.lastName}
            ...
            <label className="block">
              <span className="mb-1.5 block font-sans text-xs font-semibold uppercase tracking-[0.18em] text-forest-600">Address</span>
              <input
                type="text"
                name="address"
                value={form.address}
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <label className="block sm:col-span-2">
                <span className="mb-1.5 block font-sans text-xs font-semibold uppercase tracking-[0.18em] text-forest-600">City</span>
                <input
                  type="text"
                  name="city"
                  value={form.city}
              ...
              <label className="block">
                <span className="mb-1.5 block font-sans text-xs font-semibold uppercase tracking-[0.18em] text-forest-600">State</span>
                <input
                  type="text"
                  name="state"
                  value={form.state}
            ...
            <label className="block">
              <span className="mb-1.5 block font-sans text-xs font-semibold uppercase tracking-[0.18em] text-forest-600">ZIP Code</span>
              <input
                type="text"
                name="zip"
                value={form.zip}
            ...
            <fieldset>
              <legend className="mb-2 block font-sans text-xs font-semibold uppercase tracking-[0.18em] text-forest-600">Shipping Method</legend>
              <div className="grid gap-2">
                <label className="flex items-center justify-between rounded-xl border border-sand-300 bg-sand-50 px-3 py-2.5 font-sans text-sm text-forest-700">
                  ...
                  <span>{subtotal >= 75 || appliedPromo?.type === 'shipping' ? 'Free' : formatUSD(7)}</span>
                </label>
                <label className="flex items-center justify-between rounded-xl border border-sand-300 bg-sand-50 px-3 py-2.5 font-sans text-sm text-forest-700">
                  <span>
                    <input
            ...
            <fieldset>
              <legend className="mb-2 block font-sans text-xs font-semibold uppercase tracking-[0.18em] text-forest-600">Payment</legend>
              <div className="grid gap-2">
          ...
          <aside className="h-fit rounded-3xl border border-sand-200 bg-sand-100 p-5">
            ...
            <div className="mb-5 space-y-3">
              {lineItems.map((item) => (
                <div key={item.productId} className="flex items-center gap-3 rounded-2xl border border-sand-300 bg-sand-50 p-3">
                  <img
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-2xl font-light text-forest-900">{item.product.name}</p>
                    <p className="font-sans text-xs text-forest-500">Qty {item.quantity}</p>
                  </div>
                  <p className="font-sans text-sm font-semibold text-forest-700">{formatUSD(item.subtotal)}</p>
                </div>
              ))}
            </div>
            ...
            <div className="mb-4 rounded-2xl border border-sand-300 bg-sand-50 p-3">
              ...
              {promoError ? <p className="mt-2 font-sans text-xs text-clay-700">{promoError}</p> : null}
              {appliedPromo ? (
                ...
                <p className="mt-2 font-sans text-[11px] text-forest-500">Try: GREEN10, EARTH15, FREESHIP</p>
              )}
            </div>
